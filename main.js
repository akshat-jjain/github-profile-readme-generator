...
const generate = () => {
    ...
    const visitors_count = ($id('visitors-count').checked && input_check('github-username')) ? `<p align="left"><img src="https://komarev.com/ghpvc/?username=${github}&label=Profile%20views&color=0f3de6&style=flat-square" alt="${$id('title').value}'s Visitor Count" /></p>` : "";
    const github_trophy = ($id('github-trophy').checked && input_check('github-username')) ? `<p align="left"><a href="https://github.com/ryo-ma/github-profile-trophy"><img src="https://github-profile-trophy.vercel.app/?username=${github}" alt="${$id('title').value}'s Github Trophies" /></a></p>` : "";
    const github_metrices = ($id('github-metrices').checked && input_check('github-username')) ? `<p><img align="left" src="https://metrics.lecoq.io/${github}" alt="${$id('title').value}'s Github Metrices" /></p>` : "";
    const activity_graph = ($id('activity-graph').checked && input_check('github-username')) ? `<p><img align="center" src="https://activity-graph.herokuapp.com/graph?username=${github}" alt="${$id('title').value}'s Activity Graph" width="100%" /></p>` : "";
    const github_stats = ($id('github-stats').checked && input_check('github-username')) ? `<p><img align="center" src="https://github-readme-stats.vercel.app/api?username=${github}&show_icons=true&locale=en" alt="${$id('title').value}'s Github Stats" /></p>` : "";
    const top_skills = ($id('top-skills').checked && input_check('github-username')) ? `<p><img align="left" src="https://github-readme-stats.vercel.app/api/top-langs?username=${github}&show_icons=true&locale=en&layout=compact&langs_count=10" alt="${$id('title').value}'s Top Skills" /></p>` : "";
    const github_streak = ($id('github-streak').checked && input_check('github-username')) ? `<p><img align="center" src="https://github-readme-streak-stats.herokuapp.com/?user=${github}&theme=dark" alt="${$id('title').value}'s Github Streak" /></p>` : "";
    ...
    <p><img align="center" src="https://github-readme-stats.vercel.app/api/wakatime?username=${wakatime}&layout=compact" alt="${$id('title').value}'s Wakatime Stats" /></p>` : "";
    const twitter_badge = ($id('twitter-badge').checked && input_check('twitter-username')) ? `<p align="left"> <a href="https://twitter.com/${$id('twitter-username').value}" target="blank"><img src="https://img.shields.io/twitter/follow/${$id('twitter-username').value}?logo=twitter&style=for-the-badge" alt="${$id('title').value}'s Twitter Stats" /></a></p>` : "";
    const stackoverflow_stats = ($id('stackoverflow-stats').checked && input_check('stackoverflow-username')) ? `<h2 > My Stackoverflow Stats </h2>
    <p><img align="left" src="https://stackoverflow-card.vercel.app/?userID=${$id('stackoverflow-username').value}" alt="${$id('title').value}'s Stackoverflow Stats" /></p>` : "";
    ...
    <p><img src="https://readme-jokes.vercel.app/api?theme=default" alt="${$id('title').value}'s Jokes Card" ></p>` : "";
...
<h2> Connect with me <img src='https://raw.githubusercontent.com/ShahriarShafin/ShahriarShafin/main/Assets/handshake.gif' width="100px"> </h2>
    <p align="left">${codepen}${dev}${twitter}${linkedin}${stackoverflow}${codesandbox}${kaggle}${facebook}${instagram}${dribbble}${behance}${medium}${youtube}${codechef}${hackerrank}${codeforces}${leetcode}${hackerearth}${geeksforgeeks}${topcoder}${discord}${rss}</p>
<h2> Skills <img src = "https://media2.giphy.com/media/QssGEmpkyEOhBCb7e1/giphy.gif?cid=ecf05e47a0n3gi1bfqntqmob8g9aid1oyj2wr3ds3mg700bl&rid=giphy.gif" width = 32px></h2>
    <p align="left">${skill_dis}</p>
    ${support}
<h2> My GitHub Stats <img src='https://media1.giphy.com/media/du3J3cXyzhj75IOgvA/giphy.gif?cid=ecf05e47x2g034i9pzwtzzsd3xgg2w9nr94t4tflbbgo3008&rid=giphy.gif' width='32px'> </h2>
    ${github_metrices}${github_streak}${github_stats}${top_skills}${activity_graph}
${wakatime_stats}
${stackoverflow_stats}
${joke_card}
    `;
...
const currently_work = () => {
    return (input_check('currently-work-prefix') && input_check('currently-work')) ? `<li>${$id('currently-work-prefix').value} <strong>${$id('currently-work').value}</strong></li>` : "";
}
const looking_to = () => {
    return (input_check('looking-to-prefix') && input_check('looking-to')) ? `<li>${$id('looking-to-prefix').value} <strong>${$id('looking-to').value}</strong></li>` : "";
}
const looking_for = () => {
    return (input_check('looking-for-prefix') && input_check('looking-for')) ? `<li>${$id('looking-for-prefix').value} <strong>${$id('looking-for').value}</strong></li>` : "";
}
const currently_learning = () => {
    return (input_check('currently-learning-prefix') && input_check('currently-learning')) ? `<li>${$id('currently-learning-prefix').value} <strong>${$id('currently-learning').value}</strong></li>` : "";
}
const ask_me = () => {
    return (input_check('ask-me-prefix') && input_check('ask-me')) ? `<li>${$id('ask-me-prefix').value} <strong>${$id('ask-me').value}</strong></li>` : "";
}
const reach_me = () => {
    return (input_check('reach-me-prefix') && input_check('reach-me')) ? `<li>${$id('reach-me-prefix').value} <strong>${$id('reach-me').value}</strong></li>` : "";
}
const my_project = () => {
    return (input_check('my-project-prefix') && input_check('my-project')) ? `<li>${$id('my-project-prefix').value} <a href="${$id('my-project').value}" >${$id('my-project').value}</a></li>` : "";
}
const blog = () => {
    return (input_check('blog-prefix') && input_check('blog')) ? `<li>${$id('blog-prefix').value} <a href="${$id('blog').value}" >${$id('blog').value}</a></li>` : "";
}
const my_experience = () => {
    return (input_check('my-experience-prefix') && input_check('my-experience')) ? `<li>${$id('my-experience-prefix').value} <a href="${$id('my-experience').value}" >${$id('my-experience').value}</a></li>` : "";
}
const fun_fact = () => {
    return (input_check('fun-fact-prefix') && input_check('fun-fact')) ? `<li>${$id('fun-fact-prefix').value} <strong>${$id('fun-fact').value}</strong></li>` : "";
}
